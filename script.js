...
function moveDust(dust) {
  ...
  function animate() {
    ...
    if(menu.classList.contains('hide')) {
      ...
      dust.style.transform = `translate(${x*0.4}px, ${100+y}px)`;
    }
...
// dust img를 클릭하면 popup-post가 나타납니다.
const dustImgs = document.querySelectorAll('.dust');
const popupPosts = document.querySelectorAll('.popup-post');
  const toggleBtn = document.querySelectorAll('.switch');

dustImgs.forEach(dustImg => {
  const id = dustImg.dataset.id;
  const popupPost = document.querySelector(`.popup-post[data-id="${id}"]`);

  dustImg.addEventListener('click', () => {
    if (popupPost.classList.contains('hide')) {
